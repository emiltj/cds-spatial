...
```{r}
# List. It contains 1 element - another list of indices
```
2. Why do we need to use `neighborhoods[cont,]` to view the fully contained neighborhood in the buffer plot?
```{r}
# st_contains only produces the indexes from the original element. it doesn't create a new spatial object
```
