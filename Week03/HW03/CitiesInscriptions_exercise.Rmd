...
library(pacman)
p_load(rmdformats, knitr, sf, raster, tidyverse, leaflet, tidyverse, sf, tmap, knitr, rmdformats, raster, leaflet, tidytext, jsonlite)
...
head(cities_sf3035)
crs(cities_sf3035)
# metres, as can be read. but also projected maps are not in degrees, minutes or seconds.
```
...
# Looks really shitty to me - if you know what is wrong, please write it in the peer review.
...
i_sm <- inscriptions %>% 
  ...
  mutate(latitude = as.numeric(str_remove_all(latitude, "[)]+")), # using str_remove_all, to remove the characters I dont want
         longitude = as.numeric(str_remove_all(longitude, "[c(]+"))) 

# Check the result of the subset, does the location look reasonable?
leaflet() %>% addTiles() %>% addMarkers(lng=i_sm$longitude,lat=i_sm$latitude) # Yes it does
```
...
# Checking that longitude is in the right format, has the right numbers, and that there are no NA's
str(i[,"longitude"])
i[,"longitude"]
...
# a ratio of 7.52 (urban inscriptions, per rural inscription)
```
